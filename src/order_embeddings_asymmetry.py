...
class OrderEmbeddingTrainer:
    ...
    def compute_enhanced_loss(self, premise_embs: torch.Tensor, hypothesis_embs: torch.Tensor, 
                            labels: torch.Tensor, label_strs: List[str], margin: float = 1.5) -> Tuple[torch.Tensor, torch.Tensor, Dict]: #margin was 1.0
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'epoch': epoch,
            }, os.path.join(output_dir, "enhanced_order_embeddings_snli_10k_tests_2.pt"))
        else:
            patience_counter += 1
...
def plot_training_progress(trainer: OrderEmbeddingTrainer, save_path: str = "plots/"):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'enhanced_order_embedding_training_snli_10_tests_2.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def test_order_embeddings():
    ...
    model, trainer = train_order_embeddings(
        ...
        batch_size=32,
        order_dim=50,
        asymmetry_weight=0.3,  # Adjust this parameter (was 0.2)
        random_seed=42
    )

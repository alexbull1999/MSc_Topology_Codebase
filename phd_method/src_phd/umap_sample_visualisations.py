"""
Sample-Level UMAP Visualization for SNLI Persistence Images

Creates UMAP visualization of individual premise-hypothesis pair topological signatures
"""
...
class SampleLevelUMAPVisualizer:
    """
    Create UMAP visualizations of individual sample persistence images
    """
    ...
    def __init__(self, chunk_number=1, random_state=42):
        ...
        # UMAP parameters for ~100k samples
    ...
    def create_umap_visualization(self, persistence_images, labels, 
                                 title="Sample-Level Topological Signatures", 
                                 save_path=None, alpha=0.6):
        """
        Create UMAP visualization of individual samples
        """
        print(f"Creating UMAP visualization for {len(persistence_images)} samples...")
    ...
    def run_single_chunk_analysis(self, save_dir="phd_method/sample_level_umap"):
        """
        Run UMAP analysis on a single chunk of ~100k samples
        """
        ...
        save_path = f"{save_dir}/sample_level_umap_chunk_{self.chunk_number}.png"
        
        embedding = self.create_umap_visualization(
            images, labels,
            title=title,
            save_path=save_path,
...
def main():
    """
    Run sample-level UMAP analysis on a single chunk
    """
    
    print("Sample-Level Topological UMAP Analysis")
    print("Single Chunk Analysis (~100k samples)")
    print("="*50)
    
    # Create visualizer for chunk 1 (you can change this to any chunk 1-5)
    visualizer = SampleLevelUMAPVisualizer(
    ...
    print(f"\nSample-level UMAP analysis completed!")

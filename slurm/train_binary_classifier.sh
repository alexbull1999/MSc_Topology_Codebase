#!/bin/bash
#SBATCH --job-name=tda_binary_hyperparam_search
#SBATCH --output=logs/binary_hyperparam_search%j.out
#SBATCH --error=logs/binary_hyperparam_search%j.err
...
mkdir -p results/overnight_binary_hyperparam_search
...
python classifiers/binary_train_classifier_landmark_asymmetry.py \
    ...
    --use_random_search \
    --final_training \
    --results_dir results/overnight_binary_hyperparam_search

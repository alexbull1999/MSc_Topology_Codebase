...
class SurfaceDistanceMetricAnalyzer:
    ...
    def __init__(self, 
                 ...
                 seed: int = 42): #main is 42, seeded_2 is 101, seeded_3 is 333, seeded_4 is 444, seeded_5 is 500
        """
        Initialize analyzer with pre-processed data paths
        
        Args:
            llama_data_path: Path to processed LLAMA embeddings (.pt file from text_processing.py)
        ...
        self.embedding_spaces = [
            'llama_concat',          # Concatenated [premise||hypothesis] - joint representation
            # 'llama_difference',      # Premise - Hypothesis (relationship vector)
            ...
            # Lattice embedding spaces (All on LLAMA raw embeddings)
        ...
        print(f"LLAMA data: {llama_data_path}")
    ...
    def _load_preprocessed_data(self):
        """Load pre-processed LLAMA embeddings and order model"""
        print("Loading pre-processed data...")

        # Load LLAMA embeddings
        if not os.path.exists(self.llama_data_path):
            raise FileNotFoundError(f"LLAMA data not found: {self.llama_data_path}")

        print(f"Loading LLAMA embeddings from {self.llama_data_path}")
        ...
        print(f"  Label distribution: {self.llama_data['metadata']['label_counts']}")
        ...
        #     llama_dim=model_config['llama_dim'],
    ...
    def extract_all_embedding_spaces(self, max_samples_per_class: int = None) -> Dict[str, Dict[str, torch.Tensor]]:
        ...
        premise_embs = self.llama_data['premise_embeddings']  # Already on GPU
        hypothesis_embs = self.llama_data['hypothesis_embeddings']  # Already on GPU
        ...
        for label in data_by_class.keys():
            ...
            data_by_class[label] = {
                'premise_llama': premise_embs[indices],
                'hypothesis_llama': hypothesis_embs[indices],
                'indices': indices
            }
        ...
        for space in self.embedding_spaces:
            ...
            for label in data_by_class.keys():
                if space == 'llama_concat':
                    # OPTIMIZATION: GPU concatenation
                    space_embeddings[label] = torch.cat([
                        data_by_class[label]['premise_llama'],
                        data_by_class[label]['hypothesis_llama']
                    ], dim=1).cpu()
                    
                elif space == 'llama_difference':
                    # OPTIMIZATION: GPU subtraction
                    space_embeddings[label] = (data_by_class[label]['premise_llama'] - data_by_class[label]['hypothesis_llama']).cpu()
                    ...
                    epsilon = 1e-8
                    premise_llama = data_by_class[label]['premise_llama']
                    hypothesis_llama = data_by_class[label]['hypothesis_llama']
                    space_embeddings[label] = ((premise_llama * hypothesis_llama) / (torch.abs(premise_llama) + torch.abs(hypothesis_llama) + epsilon))

                elif space == 'lattice_order_violations':
                    premise_llama = data_by_class[label]['premise_llama']
                    hypothesis_llama = data_by_class[label]['hypothesis_llama']
                    space_embeddings[label] = (torch.maximum(torch.zeros_like(premise_llama), hypothesis_llama - premise_llama) ** 2)

                elif space == 'lattice_height':
                    premise_llama = data_by_class[label]['premise_llama']
                    hypothesis_llama = data_by_class[label]['hypothesis_llama']
                    space_embeddings[label] = (hypothesis_llama - premise_llama)

                elif space == 'lattice_subsumption':
                    epsilon = 1e-8
                    premise_llama = data_by_class[label]['premise_llama']
                    hypothesis_llama = data_by_class[label]['hypothesis_llama']
                    space_embeddings[label] = (torch.minimum(premise_llama, hypothesis_llama) / (torch.abs(hypothesis_llama) + epsilon))

                elif space == 'lattice_bidirectional_order_violations':
                    premise_llama = data_by_class[label]['premise_llama']
                    hypothesis_llama = data_by_class[label]['hypothesis_llama']
                    forward_violations = torch.maximum(torch.zeros_like(premise_llama), hypothesis_llama - premise_llama) ** 2
                    backward_violations = torch.maximum(torch.zeros_like(premise_llama), premise_llama - hypothesis_llama) ** 2
                    space_embeddings[label] = (torch.cat([forward_violations, backward_violations], dim=1))
                    ...
                    premise_llama = data_by_class[label]['premise_llama']
                    hypothesis_llama = data_by_class[label]['hypothesis_llama']
                    containment = (premise_llama * hypothesis_llama) / (torch.abs(premise_llama) + torch.abs(hypothesis_llama) + epsilon)
                    order_violations = torch.maximum(torch.zeros_like(premise_llama), hypothesis_llama - premise_llama) ** 2
                    height = hypothesis_llama - premise_llama
                    subsumption = torch.minimum(premise_llama, hypothesis_llama) / (torch.abs(hypothesis_llama) + epsilon)
                    space_embeddings[label] = (torch.cat([containment, order_violations, height, subsumption], dim=1))

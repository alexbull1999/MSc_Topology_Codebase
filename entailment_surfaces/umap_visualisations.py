"""
UMAP Visualization of Topological Entailment Signatures

Clean implementation that uses the correct functions from phdim_clustering.py
and adds UMAP visualization for the persistence images that achieved 100% clustering.
"""
...
class UMAPTopologyVisualizer:
    """
    Visualize distinct class-based topology using UMAP on persistence images
    ...
    def __init__(self, 
        ...
        # Initialize UMAP with settings for topology preservation
    ...
    def visualize_with_umap(self, persistence_images, labels, 
                           space_name, metric_name, save_path=None):
        """
        Create UMAP visualization of persistence images
        """
        print(f"Creating UMAP visualization for {space_name} + {metric_name}...")
        
        # Convert to numpy arrays
        X = np.array(persistence_images)
        y = np.array(labels)
        
        print(f"  Applying UMAP to {X.shape[0]} images of dimension {X.shape[1]}")
        
        # Apply UMAP embedding
        X_embedded = self.umap_reducer.fit_transform(X)
    ...
    def run_visualization_pipeline(self, target_spaces=None, target_metrics=None, 
                                 save_dir='entailment_surfaces/umap_visualizations'):
        """
        Run the complete pipeline for UMAP visualizations
        """
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        print("="*80)
        print("UMAP TOPOLOGICAL VISUALIZATION PIPELINE")
        print("="*80)
        ...
        try:
            ...
            print("\nStep 3: Generating UMAP visualizations...")
            ...
            for space_name in target_embeddings.keys():
                for metric_name in target_metrics:
                    ...
                    if len(persistence_images) > 0:
                        ...
                        save_path = f"{save_dir}/umap_{space_name}_{metric_name}_{timestamp}.png"
                        embedding_2d = self.visualize_with_umap(
...
def main():
    """
    Run UMAP visualizations for the best performing embedding spaces
    """
    ...
    visualizer = UMAPTopologyVisualizer(
        bert_data_path=bert_data_path,
        order_model_path=order_model_path,
    ...
    print(f"\nGenerated {len(results)} UMAP visualizations showing distinct topological signatures!")

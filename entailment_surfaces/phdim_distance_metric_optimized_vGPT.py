...
class SurfaceDistanceMetricAnalyzer:
    ...
    def __init__(self, 
        ...
        Args:
            gpt_data_path: Path to processed GPT embeddings (.pt file from text_processing.py)
        ...
        self.embedding_spaces = [
            'gpt_concat',          # Concatenated [premise||hypothesis] - joint representation
            # 'gpt_difference',      # Premise - Hypothesis (relationship vector)
            ...
            # Lattice embedding spaces (All on GPT raw embeddings)
        ...
        print(f"GPT data: {gpt_data_path}")
    ...
    def _load_preprocessed_data(self):
        """Load pre-processed GPT embeddings and order model"""
        print("Loading pre-processed data...")

        # Load GPT embeddings
        if not os.path.exists(self.gpt_data_path):
            raise FileNotFoundError(f"GPT data not found: {self.gpt_data_path}")

        print(f"Loading GPT embeddings from {self.gpt_data_path}")
        self.gpt_data = torch.load(self.gpt_data_path, map_location=self.device, weights_only=False)
        ...
        print(f"  Label distribution: {self.gpt_data['metadata']['label_counts']}")
    ...
    def extract_all_embedding_spaces(self, max_samples_per_class: int = None) -> Dict[str, Dict[str, torch.Tensor]]:
        ...
        premise_embs = self.gpt_data['premise_embeddings']  # Already on GPU
        hypothesis_embs = self.gpt_data['hypothesis_embeddings']  # Already on GPU
        ...
        for label in data_by_class.keys():
            ...
            data_by_class[label] = {
                'premise_gpt': premise_embs[indices],
                'hypothesis_gpt': hypothesis_embs[indices],
                'indices': indices
            }
            ...
            for label in data_by_class.keys():
                if space == 'gpt_concat':
                    ...
                    space_embeddings[label] = torch.cat([
                        data_by_class[label]['premise_gpt'],
                        data_by_class[label]['hypothesis_gpt']
                    ], dim=1).cpu()
                    
                elif space == 'gpt_difference':
                    # OPTIMIZATION: GPU subtraction
                    space_embeddings[label] = (data_by_class[label]['premise_gpt'] - data_by_class[label]['hypothesis_gpt']).cpu()
                    ...
                    epsilon = 1e-8
                    premise_gpt = data_by_class[label]['premise_gpt']
                    hypothesis_gpt = data_by_class[label]['hypothesis_gpt']
                    space_embeddings[label] = ((premise_gpt * hypothesis_gpt) / (torch.abs(premise_gpt) + torch.abs(hypothesis_gpt) + epsilon)).cpu()

                elif space == 'lattice_order_violations':
                    premise_gpt = data_by_class[label]['premise_gpt']
                    hypothesis_gpt = data_by_class[label]['hypothesis_gpt']
                    space_embeddings[label] = (torch.maximum(torch.zeros_like(premise_gpt), hypothesis_gpt - premise_gpt) ** 2).cpu()

                elif space == 'lattice_height':
                    premise_gpt = data_by_class[label]['premise_gpt']
                    hypothesis_gpt = data_by_class[label]['hypothesis_gpt']
                    space_embeddings[label] = (hypothesis_gpt - premise_gpt).cpu()

                elif space == 'lattice_subsumption':
                    epsilon = 1e-8
                    premise_gpt = data_by_class[label]['premise_gpt']
                    hypothesis_gpt = data_by_class[label]['hypothesis_gpt']
                    space_embeddings[label] = (torch.minimum(premise_gpt, hypothesis_gpt) / (torch.abs(hypothesis_gpt) + epsilon)).cpu()

                elif space == 'lattice_bidirectional_order_violations':
                    premise_gpt = data_by_class[label]['premise_gpt']
                    hypothesis_gpt = data_by_class[label]['hypothesis_gpt']
                    forward_violations = torch.maximum(torch.zeros_like(premise_gpt), hypothesis_gpt - premise_gpt) ** 2
                    backward_violations = torch.maximum(torch.zeros_like(premise_gpt), premise_gpt - hypothesis_gpt) ** 2
                    ...
                    premise_gpt = data_by_class[label]['premise_gpt']
                    hypothesis_gpt = data_by_class[label]['hypothesis_gpt']
                    containment = (premise_gpt * hypothesis_gpt) / (torch.abs(premise_gpt) + torch.abs(hypothesis_gpt) + epsilon)
                    order_violations = torch.maximum(torch.zeros_like(premise_gpt), hypothesis_gpt - premise_gpt) ** 2
                    height = hypothesis_gpt - premise_gpt
                    subsumption = torch.minimum(premise_gpt, hypothesis_gpt) / (torch.abs(hypothesis_gpt) + epsilon)
...
def main():
    ...
    analyzer = SurfaceDistanceMetricAnalyzer(
        gpt_data_path="/vol/bitbucket/ahb24/tda_entailment_new/snli_full_train_GPT2_MEAN_POOLING.pt",
        order_model_path="models/enhanced_order_embeddings_snli_SBERT_full.pt",
        results_dir="entailment_surfaces/results/GPT_snli_full_seed_444",
        seed=444
    )

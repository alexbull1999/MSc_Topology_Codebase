...
class EfficientStatisticalValidation:
    ...
    def __init__(self, llama_data_path: str, order_model_path: str, 
                 output_dir: str = "entailment_surfaces/statistical_validation"):
        self.llama_data_path = llama_data_path
    ...
    def _load_data(self):
        """Load LLAMA data and existing order model"""
        print("Loading LLAMA data...")
        self.llama_data = torch.load(self.llama_data_path, map_location='cpu', weights_only=False)
        
        print(f"LLAMA data loaded:")
        print(f"  Premise embeddings: {self.llama_data['premise_embeddings'].shape}")
        print(f"  Hypothesis embeddings: {self.llama_data['hypothesis_embeddings'].shape}")
        print(f"  Labels: {len(self.llama_data['labels'])}")
    ...
    def _organize_data_by_class(self):
        """Organize data by class for efficient sampling"""
        premise_embs = self.llama_data['premise_embeddings']
        hypothesis_embs = self.llama_data['hypothesis_embeddings']
        labels = self.llama_data['labels']
        ...
        for label in ['entailment', 'neutral', 'contradiction']:
            ...
            self.data_by_class[label] = {
                'premise_llama': premise_embs[indices].cpu(),
                'hypothesis_llama': hypothesis_embs[indices].cpu(),
                'indices': indices.cpu(),
    ...
    def compute_embeddings_for_combination(self, all_sample_indices: Dict, space_name: str) -> Dict:
        ...
        # Extract sampled LLAMA data for this space only
        all_sampled_llama_data = {}
        for class_name in ['entailment', 'neutral', 'contradiction']:
            all_premise_samples = []
            ...
            for run_idx in range(self.sample_size):
                for sample_indices_list in all_sample_indices[run_idx][class_name]:
                    premise_sample = class_data['premise_llama'][sample_indices_list]
                    hypothesis_sample = class_data['hypothesis_llama'][sample_indices_list]
                    
                    all_premise_samples.append(premise_sample)
                    all_hypothesis_samples.append(hypothesis_sample)
            
            # Stack all samples: [100, 1000, 768]
            all_sampled_llama_data[class_name] = {
        ...
        for class_name in ['entailment', 'neutral', 'contradiction']:
            premise_samples = all_sampled_llama_data[class_name]['premise']  # [100, 1000, 768]
            hypothesis_samples = all_sampled_llama_data[class_name]['hypothesis']  # [100, 1000, 768]
            
            if space_name == 'llama_concat':
...
def main():
    ...
    validator = EfficientStatisticalValidation(
        llama_data_path=llama_data_path,

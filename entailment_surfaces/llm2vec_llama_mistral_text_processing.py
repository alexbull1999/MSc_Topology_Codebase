...
class LLM2VecTextToEmbedding:
    """Text to embedding pipeline using LLM2Vec. Converts premise-hypothesis pairs to contextualised embeddings"""

    def __init__(self, use_mistral=False, device='cuda' if torch.cuda.is_available() else 'cpu'):
        """Initialize text processing pipeline"""
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        print(f"Loading LLM2Vec model on {device}...")

        if use_mistral:
            # Use Mistral-based model (unsupervised)
            ...
            self.model_name = "Mistral-7B-LLM2Vec-Unsupervised"
        else:
            # Use LLAMA-2-based model (unsupervised)
            base_model = "McGill-NLP/LLM2Vec-Llama-2-7b-chat-hf-mntp"
            peft_model = "McGill-NLP/LLM2Vec-Llama-2-7b-chat-hf-mntp-unsup-simcse"
            self.model_name = "LLAMA2-7B-LLM2Vec-Unsupervised"

        # hf_token = "" 

        # Load LLM2Vec model with unsupervised weights
        self.model = LLM2Vec.from_pretrained(
            base_model,
            peft_model_name_or_path=peft_model,
            pooling_mode="mean",  # Explicitly set mean pooling (default anyway)
        ...
        print("LLM2Vec text processing pipeline ready")
        print(f"Model automatically handles sentence embedding with mean pooling")

    def encode_text(self, texts: List[str], batch_size: int = 8) -> torch.Tensor:
        """Convert list of texts to LLM2Vec sentence embeddings
        
        Note: LLM2Vec automatically handles mean pooling internally,
        so this returns complete sentence embeddings, not token embeddings.
        
        Args:
            texts (List[str]): list of texts to encode
            batch_size (int, optional): batch size. Defaults to 8 (conservative for LLAMA-2-7B).
        Returns:
            torch.Tensor: Sentence embeddings (of shape (n_texts, hidden_size))
        """
        all_embeddings = []
        ...
        for i in range(0, len(texts), batch_size):
            ...
            # LLM2Vec.encode() returns sentence embeddings directly
            with torch.no_grad():
...
def test_llm2vec_processing():
    """Test LLM2Vec processing pipeline on toy data"""
    
    # You can choose between LLAMA-2 or Mistral
    processor = LLM2VecTextToEmbedding(use_mistral=True)  # Set to True for Mistral
    ...
    if os.path.exists(data_path):
        ...
        # Save processed data
        output_path = f"/vol/bitbucket/ahb24/tda_entailment_new/mnli_full_train_LLM2Vec_MISTRAL_SUP.pt"
        processor.save_processed_data(processed_data, output_path)

        print("LLM2Vec text processing pipeline test completed successfully")
        return processed_data
    else:
        print("LLM2Vec text processing pipeline test failed - dataset not found")

if __name__ == "__main__":
    # Install required packages first:
    # pip install llm2vec
    # pip install flash-attn --no-build-isolation
    test_llm2vec_processing()
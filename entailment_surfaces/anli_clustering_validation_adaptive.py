...
def persistence_diagrams_to_images_with_params(all_diagrams, adaptive_params):
...
def analyze_diagram_ranges(all_diagrams):
    ...
    for space_name in embedding_spaces:
        ...
        for metric in metrics:
            ...
            for run_idx in range(n_samples):
                ...
                sample_labels = []
                collected_diagrams = []  # Collect all diagrams for adaptive params
                ...
                for class_name in ['entailment', 'neutral', 'contradiction']:
                    ...
                    for sample_idx in range(10):
                        ...
                        if len(embeddings) > 1000:
                            indices = torch.randperm(len(embeddings))[:1000]
                        ...
                        try:
                            ...
                            ph_dim, all_diagrams = ph_dim_and_diagrams_from_distance_matrix(
                                distance_matrix,
                                min_points=200,
                                max_points=1000,  # Hardcoded to 1000
                                point_jump=50,
                                h_dim=0,
                                alpha=1.0
                            )
                            
                            # Collect diagrams for adaptive parameter calculation
                            if len(all_diagrams) > 0:
                                collected_diagrams.append(all_diagrams[0])
                            
                        except Exception as e:
                            print(f"    Run {run_idx+1}, {class_name}, sample {sample_idx+1}: Error - {e}")
                
                # Calculate adaptive parameters from all collected diagrams
                if len(collected_diagrams) > 0:
                    adaptive_params = analyze_diagram_ranges(collected_diagrams)
                    
                    # Convert all diagrams to persistence images with adaptive params
                    all_persistence_images = persistence_diagrams_to_images_with_params(
                        collected_diagrams, adaptive_params
                    )
                    
                    # Reconstruct sample labels (same order as collected_diagrams)
                    sample_labels = []
                    for class_name in ['entailment', 'neutral', 'contradiction']:
                        class_idx = label_to_idx[class_name]
                        embeddings = full_embeddings[class_name]
                        if len(embeddings) >= 200:
                            sample_labels.extend([class_idx] * 10)  # 10 samples per class
...
if __name__ == "__main__":
    ...
    output_file = f"{output_dir}/anli_clustering_validation_train_seed{SEED}_ADAPTIVE.json"

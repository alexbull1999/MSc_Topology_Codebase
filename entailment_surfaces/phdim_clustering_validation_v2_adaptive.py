...
class EfficientStatisticalValidation:
    ...
    def __init__(self, sbert_data_path: str, order_model_path: str, 
    ...
    def _load_data(self):
        """Load SBERT data and existing order model"""
        print("Loading SBERT data...")
        self.sbert_data = torch.load(self.sbert_data_path, map_location='cpu', weights_only=False)
        
        print(f"SBERT data loaded:")
        print(f"  Premise embeddings: {self.sbert_data['premise_embeddings'].shape}")
        print(f"  Hypothesis embeddings: {self.sbert_data['hypothesis_embeddings'].shape}")
        print(f"  Labels: {len(self.sbert_data['labels'])}")
    ...
    def _organize_data_by_class(self):
        """Organize data by class for efficient sampling"""
        premise_embs = self.sbert_data['premise_embeddings']
        hypothesis_embs = self.sbert_data['hypothesis_embeddings']
        labels = self.sbert_data['labels']
        ...
        for label in ['entailment', 'neutral', 'contradiction']:
            ...
            self.data_by_class[label] = {
                'premise_sbert': premise_embs[indices].cpu(),
                'hypothesis_sbert': hypothesis_embs[indices].cpu(),
                'indices': indices.cpu(),
    ...
    def compute_embeddings_for_combination(self, all_sample_indices: Dict, space_name: str) -> Dict:
        ...
        # Extract sampled SBERT data for this space only
        all_sampled_sbert_data = {}
        for class_name in ['entailment', 'neutral', 'contradiction']:
            all_premise_samples = []
            ...
            for run_idx in range(self.sample_size):
                for sample_indices_list in all_sample_indices[run_idx][class_name]:
                    premise_sample = class_data['premise_sbert'][sample_indices_list]
                    hypothesis_sample = class_data['hypothesis_sbert'][sample_indices_list]
                    
                    all_premise_samples.append(premise_sample)
                    all_hypothesis_samples.append(hypothesis_sample)
            
            # Stack all samples: [100, 1000, 768]
            all_sampled_sbert_data[class_name] = {
        ...
        for class_name in ['entailment', 'neutral', 'contradiction']:
            premise_samples = all_sampled_sbert_data[class_name]['premise']  # [100, 1000, 768]
            hypothesis_samples = all_sampled_sbert_data[class_name]['hypothesis']  # [100, 1000, 768]

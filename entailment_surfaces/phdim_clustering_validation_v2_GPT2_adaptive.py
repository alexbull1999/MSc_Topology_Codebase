...
def persistence_diagrams_to_images_with_params(all_diagrams: List, adaptive_params: Dict) -> List[np.ndarray]:
    """Convert persistence diagrams to standardized images with given parameters"""
    
    pimgr = PersistenceImager(
        pixel_size=adaptive_params['pixel_size'],
        birth_range=adaptive_params['birth_range'],
        pers_range=adaptive_params['pers_range'],
        kernel_params={'sigma': adaptive_params['sigma']}
    )
...
def analyze_diagram_ranges(all_diagrams: List) -> Dict:
    """Analyze diagrams to determine optimal parameters"""
    all_births = []
    all_deaths = []
    all_persistences = []
    ...
    for diagrams in all_diagrams:
        for dim_diagram in diagrams:
            finite_points = dim_diagram[np.isfinite(dim_diagram).all(axis=1)]
            if len(finite_points) > 0:
                all_births.extend(finite_points[:, 0])
                all_deaths.extend(finite_points[:, 1])
                all_persistences.extend(finite_points[:, 1] - finite_points[:, 0])
    
    if not all_births:
        return {'birth_range': (0, 5), 'pers_range': (0, 5), 'pixel_size': 0.5, 'sigma': 0.3}
    
    birth_min, birth_max = np.min(all_births), np.max(all_births)
    pers_min, pers_max = np.min(all_persistences), np.max(all_persistences)
    
    # Add padding
    birth_padding = 0.1 * (birth_max - birth_min) if birth_max > birth_min else 0.5
    pers_padding = 0.1 * (pers_max - pers_min) if pers_max > pers_min else 0.5
    
    birth_range = (max(0, birth_min - birth_padding), birth_max + birth_padding)
    pers_range = (max(0, pers_min - pers_padding), pers_max + pers_padding)
    
    # Adaptive resolution
    pixel_size = (pers_range[1] - pers_range[0]) / 20
    sigma = pixel_size * 0.6
    
    print(f"\nAdaptive parameters calculated:")
    print(f"  Birth range: {birth_range}")
    print(f"  Persistence range: {pers_range}")
    print(f"  Pixel size: {pixel_size:.3f}")
...
class EfficientStatisticalValidation:
    ...
    def __init__(self, gpt_data_path: str, order_model_path: str, 
                 output_dir: str = "entailment_surfaces/statistical_validation"):
        self.gpt_data_path = gpt_data_path
        ...
        self.embedding_spaces = ['gpt_concat', 'lattice_containment']
        
        # All distance metrics for all 4 spaces as requested
        self.distance_metrics = [
            'euclidean', 'manhattan', 'chebyshev', 'cosine', 'minkowski_3', 'minkowski_4', 'braycurtis'
        ]
    ...
    def _load_data(self):
        """Load GPT data and existing order model"""
        print("Loading GPT data...")
        self.gpt_data = torch.load(self.gpt_data_path, map_location='cpu', weights_only=False)
        
        print(f"GPT data loaded:")
        print(f"  Premise embeddings: {self.gpt_data['premise_embeddings'].shape}")
        print(f"  Hypothesis embeddings: {self.gpt_data['hypothesis_embeddings'].shape}")
        print(f"  Labels: {len(self.gpt_data['labels'])}")
    ...
    def _organize_data_by_class(self):
        """Organize data by class for efficient sampling"""
        premise_embs = self.gpt_data['premise_embeddings']
        hypothesis_embs = self.gpt_data['hypothesis_embeddings']
        labels = self.gpt_data['labels']
        
        self.data_by_class = {}
        
        for label in ['entailment', 'neutral', 'contradiction']:
            mask = torch.tensor([l == label for l in labels], dtype=torch.bool)
            indices = torch.where(mask)[0]
            
            self.data_by_class[label] = {
                'premise_gpt': premise_embs[indices].cpu(),
                'hypothesis_gpt': hypothesis_embs[indices].cpu(),
                'indices': indices.cpu(),
    ...
    def compute_embeddings_for_combination(self, all_sample_indices: Dict, space_name: str) -> Dict:
        ...
        # Extract sampled GPT data for this space only
        all_sampled_gpt_data = {}
        for class_name in ['entailment', 'neutral', 'contradiction']:
            all_premise_samples = []
            ...
            for run_idx in range(self.sample_size):
                for sample_indices_list in all_sample_indices[run_idx][class_name]:
                    premise_sample = class_data['premise_gpt'][sample_indices_list]
                    hypothesis_sample = class_data['hypothesis_gpt'][sample_indices_list]
                    
                    all_premise_samples.append(premise_sample)
                    all_hypothesis_samples.append(hypothesis_sample)
            
            # Stack all samples: [100, 1000, 768]
            all_sampled_gpt_data[class_name] = {
        ...
        for class_name in ['entailment', 'neutral', 'contradiction']:
            premise_samples = all_sampled_gpt_data[class_name]['premise']  # [100, 1000, 768]
            hypothesis_samples = all_sampled_gpt_data[class_name]['hypothesis']  # [100, 1000, 768]
            
            if space_name == 'gpt_concat':
    ...
    def run_single_sample_validation(self, space_embeddings: Dict, space_name: str, metric_name: str, run_idx: int, adaptive_params: Dict) -> Dict:
        ...
        for class_name in ['entailment', 'neutral', 'contradiction']:
            ...
            for sample_idx in range(start_idx, end_idx):
                ...
                if len(all_diagrams) > 0:
                    persistence_image = persistence_diagrams_to_images_with_params([all_diagrams[0]], adaptive_params)
    ...
    def run_statistical_validation(self) -> Dict:
        ...
        for combo_idx, (space_name, metric_name) in enumerate(self.top_combinations):
            ...
            if combo_idx == 0:  # Only do this for the first combination to avoid spam
                self.diagnose_persistence_diagrams(space_embeddings, space_name, metric_name)
            
            # CALCULATE ADAPTIVE PARAMETERS ONCE FOR THIS COMBINATION
            adaptive_params = self.calculate_optimal_parameters(space_embeddings, metric_name)
            print(f"Using consistent parameters for all runs: {adaptive_params}")
            
            # Run all 10 validation runs for this combination with CONSISTENT parameters
            sample_results = []
            for run_idx in range(self.sample_size):
                print(f"  Run {run_idx + 1}/{self.sample_size}...")
                
                result = self.run_single_sample_validation(
                    space_embeddings, space_name, metric_name, run_idx, adaptive_params
                )
    ...
    def diagnose_persistence_diagrams(self, space_embeddings: Dict, space_name: str, metric_name: str):
        """Examine raw persistence diagrams to understand data characteristics"""
        print(f"\n=== DIAGNOSING PERSISTENCE DIAGRAMS: {space_name} + {metric_name} ===")
        
        for class_name in ['entailment', 'neutral', 'contradiction']:
            class_embeddings = space_embeddings[class_name]
            sample_embedding = class_embeddings[0]  # First run, first sample
            ...
            for dim in range(len(diagrams)):
                ...
                if len(finite_diagram) > 0:
                    ...
                    print(f"  H{dim}: {len(finite_diagram)} features")
                    print(f"    Birth range: [{births.min():.3f}, {births.max():.3f}]")
                    print(f"    Death range: [{deaths.min():.3f}, {deaths.max():.3f}]")
                    print(f"    Persistence range: [{persistences.min():.3f}, {persistences.max():.3f}]")
                    print(f"    Mean persistence: {persistences.mean():.3f}")

    def calculate_optimal_parameters(self, space_embeddings: Dict, metric_name: str) -> Dict:
        """Calculate optimal parameters once per combination using sample diagrams"""
        
        # Collect diagrams from a few samples across all classes
        sample_diagrams = []
        for class_name in ['entailment', 'neutral', 'contradiction']:
            # Take first 3 samples from this class
            for sample_idx in range(min(3, space_embeddings[class_name].shape[0])):
                sample_embedding = space_embeddings[class_name][sample_idx]
                distance_matrix = compute_distance_matrix_advanced(sample_embedding, metric_name)
                diagrams = ripser_parallel(distance_matrix, maxdim=1, n_threads=-1, metric="precomputed")['dgms']
                sample_diagrams.append(diagrams)
        
        # Calculate parameters from all sample diagrams
        return analyze_diagram_ranges(sample_diagrams)


def main():
    """Run efficient statistical validation"""
    
    gpt_data_path = "/vol/bitbucket/ahb24/tda_entailment_new/mnli_full_train_GPT2_MEAN_POOLING.pt"
    order_model_path = "models/enhanced_order_embeddings_snli_SBERT_full.pt"
    
    validator = EfficientStatisticalValidation(
        gpt_data_path=gpt_data_path,
